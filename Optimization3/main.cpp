...
void spaceship_shot(int x,int y) {
     shots.push_back(new Figure(x+4,y+3,3,5,shotStructure(5),false));
     shots.push_back(new Figure(x+23,y+3,3,5,shotStructure(5),false));
}

void create_enemies() {
    enemies.push_back(new Figure(10,10,30,28,spaceshipStructure(28),true));
    enemies.push_back(new Figure(45,10,30,28,spaceshipStructure(28),true));
    enemies.push_back(new Figure(80,10,30,28,spaceshipStructure(28),true));
    enemies.push_back(new Figure(115,10,30,28,spaceshipStructure(28),true));
    enemies.push_back(new Figure(150,10,30,28,spaceshipStructure(28),true));
}
...
void deleteEnemy(int x) {
    delete enemies[x-2];
    enemies.erase(enemies.begin()+x-2);
    collisionHandler->figures.erase(collisionHandler->figures.begin()+x-1);
}
...
void moving_shots() {
     if(!shots.empty()) {
         for(unsigned int i=0;i<shots.size();i++) {
                if((unsigned)shots[i]->y==7) {
                    delete shots[i];
                    shots.erase(shots.begin()+i);
                    i--;
                } else {
                    shots[i]->y=shots[i]->y-2;
                    collisionHandler->shotImpactCheck(shots[i]);
                    if(collisionHandler->collitions>0) {
                        delete shots[i];
                        shots.erase(shots.begin()+i);
                        deleteEnemy(collisionHandler->collitions);
                        collisionHandler->collitions=0;
                    }
                }
         }
     }
...
void main_paint() {
     ...
     spaceship=new Figure(HORIZONTAL/(2*HORSPACE)-15,VERTICAL/VERSPACE-10*VERSPACE-14,30,28,spaceshipStructure(28),true);
     create_enemies();
